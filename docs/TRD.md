...
| `CRON_SECRET` | Shared bearer token between node-cron and the processor endpoints | Random string |
| `SEQUENCE_CRON_ENABLED` | Kill switch for the in-container cron (sequences, drafts, archive, zoom). `true` enables | `true` |
| `NEXT_PUBLIC_ADMIN_URL` | Admin base URL the cron's self-call hits | `https://leadsportaladmin.kitlabs.us` |
| `ZOOM_ACCOUNT_ID` | Zoom Server-to-Server OAuth account id (optional — when unset, Zoom provisioning is a no-op) | from Zoom Marketplace |
| `ZOOM_CLIENT_ID` | Zoom OAuth client id | from Zoom Marketplace |
| `ZOOM_CLIENT_SECRET` | Zoom OAuth client secret | from Zoom Marketplace |
| `ZOOM_HOST_USER_ID` | (Optional) Zoom user who hosts created meetings. Defaults to `"me"` (the OAuth app owner) | `me` or a Zoom user id |
| `APP_FACTORY_SECRET_KEY` | AES-256-GCM key for encrypting AppStoreConfig.apiKey at rest. Base64-encoded 32 bytes | `openssl rand -base64 32` |
| `AWS_S3_BUCKET` | S3 bucket for lead documents | `kitlabs-leads-portal-documents` |
| `AWS_S3_REGION` | S3 region | `us-east-1` |
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` | Google OAuth for customer portal sign-in | from Google Cloud Console |
| `ANTHROPIC_API_KEY` | Claude API key for SOW + app-flow generation | from Anthropic Console |

**Security:** The `.env` file is listed in both `.gitignore` and `.dockerignore` to prevent secrets from being committed or included in Docker images. Production environment variables are written to EC2's `/home/ubuntu/leads-portal/.env` by `.github/workflows/deploy.yml` from GitHub Actions secrets.
...
| 4.12 | April 8, 2026 | EmailTemplate model: sendAfterDays Int? field for timing. EmailTemplatePurpose enum: 2 new values (NURTURE, COLD_OUTREACH). Templates list: body preview snippet (80 chars), Delay column (Day N badge), Clone button (creates copy with "(Copy)" suffix). Create/Edit forms: Send After (Days) numeric field, 2 new Purpose options. FlowBuilder: nodes show send delay badge, sidebar templates show delay pill. Clone button on edit page header | — |
| 5.0 | May 19, 2026 | **Native meeting booking + Zoom + sequence observability + Email Flow removal**. New models: MeetingType, MeetingBooking (with zoomMeetingId + conferencingError + conferencingAttempts + conferencingNotifiedAt). Customer portal: public `/book` page (no auth, `?leadId=` deep-link), in-portal `/project?tab=appointments` uses same `<BookingForm/>`. 3 public customer APIs: /api/meetings/types, /api/meetings/availability, /api/meetings/book. Admin: `/meetings` page with Bookings + Meeting Types tabs, 4 admin APIs (`/api/meeting-types[/id]`, `/api/admin-meetings[/id]`). Availability logic: Mon-Fri 9-5 ET, 14-day horizon, 60-min lead, double-book guard. Zoom Server-to-Server OAuth integration: `apps/admin/src/lib/zoom.ts` (token cache + createZoomMeeting + cancelZoomMeeting), cron tick every 2 min via `/api/meetings/provision-zoom` (added to middleware PUBLIC_PATHS), retries 4× before `conferencingError`. `.ics` calendar attachments on all booking emails (METHOD:REQUEST, deterministic UID, SEQUENCE:0 → 1 on Zoom link arrival). Sender display name resolves to lead.assignedTo.name or "KITLabs Meetings" fallback (`getFromAddress` helper duplicated in both portals). New merge tags: first_name, company_name, customerPortalUrl, bookMeetingUrl, projectBookingUrl — wired through template-merge.ts + manual compose + sample data on /email-templates + /sequences/[id] previews. TemplateTags palette refreshed with 15 tags grouped by Contact / Project / Links. Smart Sequences gained Flowchart tab (read-only) and Audit Log tab; `GET /api/sequences/[id]/audit` paginated. Post-tick summary email: `sendSequenceTickSummary` + new `sequenceActivity` NotificationPreference (default ON, exposed at /notification-settings). Processor poison-pill fix: pauses enrollments whose parent sequence is non-ACTIVE. Lead detail: upcoming-meeting card at top of right column (lead.meetingBookings now included in GET); Additional Contacts panel moved under Project Details + auto-CC'd in compose form. **Email Flows removed**: deleted /email-flows pages (list/new/[id]), /api/email-flows routes, FlowBuilder component, /api/leads/[id]/recommendations endpoint + the "Recommended Next Email" card on lead detail, EmailFlow Prisma model. Customer portal: ICS generator in apps/customer/src/lib/ics.ts. Admin: booking-email.ts (sendZoomLinkEmail). Schema additions: lead.meetingBookings backref; deploy.yml + docker-compose.prod.yml pass ZOOM_* env vars | — |
...
| 4.16 | April 10, 2026 | Phase 2 production hardening: SES bounce/complaint handling (hard bounce → doNotContact + exit sequences + audit, complaint → same, soft bounce → log only), click tracking (rewriteLinksForTracking + /api/track-click/[id] redirect + SentEmail.clickedAt + lastAction=CLICKED), auto-enrollment triggers (LEAD_CREATED hook in POST /api/leads, STAGE_CHANGE hook in PATCH /api/leads/[id]/status with triggerConfig matching, shared enrollment-utils.ts), EmailDraft scheduled sending (/api/drafts/process with claim-lock, lockedUntil+retryCount+failureReason on EmailDraft, cron every 5 min), SystemHealth singleton table for cron heartbeat (tickWithHealth wrapper, GET /api/health 200/503, dashboard systemHealth widget), data-no-track on unsubscribe links to skip click rewriting | — |
| 4.15 | April 9, 2026 | Smart Sequence email sending activated: node-cron in admin container (instrumentation.ts startup hook) self-calls /api/sequences/process every minute and /api/sequences/archive-old daily at 3 AM UTC. Crash-safe processor with SELECT FOR UPDATE SKIP LOCKED claim-and-lock (5-min lock), idempotency unique constraint on SentEmail(enrollmentId, enrollmentStep), advance-before-send order (crash = missed not duplicate), retry counter (max 5, 10-min backoff, EXITED on giveup), 80ms send pacing (~14/sec for SES). Schema: lockedUntil + retryCount on SequenceEnrollment, SequenceEnrollmentArchive cold-storage table, partial index `WHERE status='ACTIVE'` (raw SQL in start-admin.sh). Branching gap fixes: tracking pixel updates lastAction=OPENED, SES inbound webhook updates lastAction=REPLIED. 90-day archival of completed/exited enrollments. New shared template-merge.ts util with 10 standard merge tags. Dockerfile adds postgresql-client. docker-compose plumbs CRON_SECRET + SEQUENCE_CRON_ENABLED + NEXT_PUBLIC_ADMIN_URL. deploy.yml writes new secrets to EC2 .env | — |
| 4.14 | April 9, 2026 | Contact Lists: 2 new enums (ListType, MembershipSource), 2 new models (ContactList, ListMembership), ADDED_TO_LIST enrollment trigger on SmartSequence with triggerListId FK, 5 API routes (lists CRUD, membership add/remove with auto-enroll, dynamic list refresh with Prisma where builder, bulk sequence enrollment with DNC+suppression checks), 3 admin pages (/lists index with type filter+search, /lists/new with static/dynamic type selector+filter rule builder, /lists/[id] with Contacts/Sequences/Settings tabs), list-utils.ts with buildPrismaWhereFromFilters() and 12 filter field definitions, suppression list support, enrollment decoupling rule | — |
| 4.13 | April 8, 2026 | Smart Sequences: 7 new enums (SequenceGoal, SequenceStatus, EnrollmentTrigger, WaitUnit, StepCondition, EnrollmentStatus, ContactAction), 3 new models (SmartSequence, SequenceStep, SequenceEnrollment), 8 API routes (sequences CRUD, batch step save, enrollments with pause/resume/remove/advance, plain-language preview, performance metrics, cron processor with SMTP sending), 3 admin pages (/sequences list, /sequences/new create, /sequences/[id] detail with Steps/Contacts/Preview/Performance tabs), drag-to-reorder step cards, structured delay (number + unit), branching conditions (always/opened/clicked/replied), step-level exit conditions, sequence-level exit conditions, contact enrollment with lead search modal, sequence-utils.ts helpers, sidebar nav item | — |
...
| 4.7 | April 7, 2026 | EmailTemplate model: 3 new fields (industry, naicsSectorCode, naicsSubsectorCode), NAICS cascading dropdowns on create + edit forms, title label renamed to "Template Name (internal reference)", Industry/NAICS fields in both POST and PUT API handlers | — |
