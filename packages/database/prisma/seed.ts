...
const systemEmailTemplates: SystemTemplateSeed[] = [
  {
...
async function seedSystemEmailTemplates() {
  for (const tpl of systemEmailTemplates) {
    const existing = await prisma.emailTemplate.findUnique({
      where: { systemKey: tpl.systemKey },
    });

    if (existing) {
      console.log(`System template "${tpl.systemKey}" already exists, skipping.`);
      continue;
    }
    ...
    console.log(`Seeded system template: ${tpl.systemKey}`);
  }
}
